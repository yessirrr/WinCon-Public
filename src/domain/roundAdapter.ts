...
export interface RoundObs {
  roundIndex: number;           // 1-based within map, OT as 25+
  halfTag: HalfTag;
  mapName: string;
  attackTeamId: string;
 ...
 */
function parseDurationToSec(duration?: string): number | undefined {
 ...
 */
function getHalfTag(roundIndex: number): HalfTag {
  if (roundIndex <= FIRST_HALF_END) return 'H1';
  if (roundIndex < OVERTIME_START) return 'H2';
  return 'OT';
}
 ...
 */
function getTeamsBySide(round: Round): { attackTeamId: string; defenseTeamId: string } | null {
  let attackTeamId: string | null = null;
  let defenseTeamId: string | null = null;

  for (const [teamId, side] of Object.entries(round.side)) {
    if (side === 'attacker') attackTeamId = teamId;
    else if (side === 'defender') defenseTeamId = teamId;
  }

  if (!attackTeamId || !defenseTeamId) return null;
 ...
 */
function countDeathsByTeam(round: Round): Record<string, number> {
  const deaths: Record<string, number> = {};

  for (const ps of round.playerStats) {
    if (!deaths[ps.teamId]) deaths[ps.teamId] = 0;
 ...
 */
export function roundToObs(round: Round, mapName: string): RoundObs | null {
 ...
 */
export function mapToRoundObs(map: MapResult): RoundObs[] {
  const observations: RoundObs[] = [];
 ...
 */
export function isPistolRound(roundIndex: number): boolean {
 ...
 */
export function isPostPistolRound(roundIndex: number): boolean {
 ...
 */
export function isHalfStart(roundIndex: number): boolean {
