...
export async function exportToMarkdown(report: Report): Promise<string> {
...
function generateMarkdown(report: Report): string {
  ...
  if (report.filters) {
    ...
    if (report.filters.window) filterParts.push(`Last ${report.filters.window} matches`);
    if (report.filters.map) filterParts.push(`Map: ${report.filters.map}`);
...
function renderContent(content: ReportContent): string {
  switch (content.type) {
    ...
    case 'momentum':
      return renderMomentum(content.data as RoundOutcomeReport);

    case 'econ-intel':
      return renderEconIntel(content.data as EconIntelData);
...
function renderMomentum(data: RoundOutcomeReport): string {
  const lines: string[] = [];
...
function renderEconIntel(data: EconIntelData): string {
  const lines: string[] = [];
