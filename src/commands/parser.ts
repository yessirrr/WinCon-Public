...
export function classifyCommand(name: string): CommandType {
  ...
  if (navigation.includes(name)) return 'navigation';
  if (report.includes(name)) return 'report';
  if (filter.includes(name)) return 'filter';
  if (system.includes(name)) return 'system';
