...
const GRID_STATE_GQL_ENDPOINT =
  process.env.GRID_STATE_GQL_ENDPOINT || 'https://api-op.grid.gg/live-data-feed/series-state/graphql';
// Statistics Feed (aggregated stats)
const GRID_STATS_GQL_ENDPOINT =
  process.env.GRID_STATS_GQL_ENDPOINT || 'https://api-op.grid.gg/statistics-feed/graphql';
...
async function executeGridQuery<T>(endpoint: string, query: string, variables?: Record<string, unknown>): Promise<T> {
  const maxAttempts = 3;
...
export function queryGrid<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  return executeGridQuery<T>(GRID_GQL_ENDPOINT, query, variables);
}

// Series-state GraphQL (live / player participation)
export function queryGridState<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  return executeGridQuery<T>(GRID_STATE_GQL_ENDPOINT, query, variables);
}

// Statistics-feed GraphQL (aggregated stats)
export async function queryGridStats<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  ...
  try {
    const data = await executeGridQuery<T>(GRID_STATS_GQL_ENDPOINT, query, variables);
    if (debug) {
...
// VCT 2025 Partnered Teams - Hardcoded for instant browsing
// Players and stats are fetched from GRID API on-demand
...
export const VCT_TEAMS: VctTeam[] = [
  // ==================== AMERICAS ====================
...
export function getVctTeamsByRegion(region: VctRegion): VctTeam[] {
  return VCT_TEAMS.filter(t => t.region === region);
}

// Find team by name (case-insensitive, partial match)
export function findVctTeam(name: string): VctTeam | undefined {
  const lower = name.toLowerCase();
  const normalized = normalizeTeamLookupValue(name);
  const exact = VCT_TEAMS.find(t =>
...
export function vctRegionToRegion(vctRegion: VctRegion): 'NA' | 'EMEA' | 'APAC' | 'CN' {
  switch (vctRegion) {
...
export function getTeamRegion(teamName: string): 'NA' | 'EMEA' | 'APAC' | 'CN' | undefined {
...
export function isApiConfigured(): boolean {
  if (isDemoMode()) return false; // Never call API in demo mode
