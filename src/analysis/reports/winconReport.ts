import type { Report, ReportFilters, ReportSection, Match } from '../../data/models/index.js';
...
interface SideIdentityMapStats {
  mapName: string;
  attack: PosteriorSummary;
  defense: PosteriorSummary;
  probDeltaGt: number | null;
  probDeltaLt: number | null;
  identity: 'Attack-Sided' | 'Defense-Sided' | 'Balanced' | 'N/A';
}
...
interface ClosingStats {
  ...
  closingCoefficient: number | null;
  predictedStates: Array<{ label: string; winProb: number | null }>;
  sampleCount: number;
}

interface PlayerDependenceRow {
  playerId: string;
  playerName: string;
  goodWinRate: number | null;
  goodCiLow: number | null;
  goodCiHigh: number | null;
  badWinRate: number | null;
  badCiLow: number | null;
  badCiHigh: number | null;
  lift: number | null;
...
function posteriorSummary(wins: number, rounds: number, credLevel: number, seedLabel: string): PosteriorSummary {
  const alpha = wins + 1;
...
function estimateDeltaProbabilities(
  attack: PosteriorSummary,
  defense: PosteriorSummary,
  deltaThreshold: number,
  seedLabel: string
): { probGt: number | null; probLt: number | null } {
  ...
  for (let i = 0; i < BETA_SAMPLES; i += 1) {
    ...
    const delta = a - d;
    if (delta > deltaThreshold) gt += 1;
    if (delta < -deltaThreshold) lt += 1;
  }
  return { probGt: gt / BETA_SAMPLES, probLt: lt / BETA_SAMPLES };
}

function applyFilters(matches: Match[], filters: ReportFilters): Match[] {
  if (!filters.map) return matches;
...
function coefficientPerUnit(model: LogisticModel, featureName: string): number | null {
  const idx = model.featureNames.indexOf(featureName);
  if (idx === -1) return null;
  return model.coefficients[idx + 1] / model.stds[idx];
}

function computeSideIdentity(matches: Match[], teamId: string, filters: ReportFilters): SideIdentityMapStats[] {
  const mapStats = new Map<string, { attack: { wins: number; rounds: number }; defense: { wins: number; rounds: number } }>();
  ...
  for (const match of matches) {
    for (const map of match.maps) {
      for (const round of map.rounds) {
        ...
        if (!side) continue;
        if (filters.side && filters.side !== 'both' && side !== filters.side) continue;
        ...
        bucket.rounds += 1;
        if (round.winnerId === teamId) bucket.wins += 1;
        mapStats.set(key, entry);
      }
    }
  }

  const stats: SideIdentityMapStats[] = [];
  for (const [mapName, data] of mapStats.entries()) {
    const attackSummary = posteriorSummary(
      data.attack.wins,
      data.attack.rounds,
      SIDE_CRED_LEVEL,
      `${mapName}-attack-${data.attack.wins}-${data.attack.rounds}`
    );
    const defenseSummary = posteriorSummary(
      data.defense.wins,
      data.defense.rounds,
      SIDE_CRED_LEVEL,
      `${mapName}-defense-${data.defense.wins}-${data.defense.rounds}`
    );

    let identity: SideIdentityMapStats['identity'] = 'Balanced';
    let probDeltaGt: number | null = null;
    let probDeltaLt: number | null = null;
    if (attackSummary.rounds === 0 || defenseSummary.rounds === 0) {
      ...
      if (probDeltaGt !== null && probDeltaGt > 0.8) identity = 'Attack-Sided';
      if (probDeltaLt !== null && probDeltaLt > 0.8) identity = 'Defense-Sided';
    } else {
      identity = 'N/A';
    }
...
function computeClosingAbility(matches: Match[], teamId: string): ClosingStats {
  let opportunities = 0;
  let converted = 0;
  ...
  for (const match of matches) {
    for (const map of match.maps) {
      ...
      let leadAt10: number | null = null;
      let momentumAt10: number | null = null;
...
function computePlayerDependence(matches: Match[], teamId: string): PlayerDependenceRow[] {
  const mapEntries: Array<{
    mapName: string;
    teamWon: boolean;
    playerStats: Array<{ playerId: string; playerName: string; kdDiff: number }>;
  }> = [];
  ...
  for (const match of matches) {
    for (const map of match.maps) {
      ...
      const playerTotals = new Map<string, { name: string; kills: number; deaths: number }>();
      for (const round of map.rounds) {
  ...
  const playerTotals = new Map<string, { name: string; kdDiffs: number[] }>();
  ...
  const rows: PlayerDependenceRow[] = [];
  for (const [playerId, totals] of playerTotals.entries()) {
    ...
    for (const entry of mapEntries) {
      ...
      if (isGood) {
        goodTotal += 1;
        if (entry.teamWon) goodWins += 1;
      } else {
        badTotal += 1;
        if (entry.teamWon) badWins += 1;
      }
    }
  ...
  const featureNames: string[] = [...playerIds.map(id => `impact:${id}`)];
...
let cachedSummaryTemplates: SummaryTemplates | null = null;

function loadSummaryTemplates(): SummaryTemplates {
  if (cachedSummaryTemplates) return cachedSummaryTemplates;
  const templatePath = path.join(getAppRoot(), 'data', 'raw', 'Summary_Template.md');
  const templates: SummaryTemplates = { side: [], closing: [], player: [], focus: [] };
  let current: keyof SummaryTemplates | null = null;
  ...
  try {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      if (trimmed.startsWith('## ')) {
        ...
        if (lower.includes('side-half')) current = 'side';
        else if (lower.includes('closing ability')) current = 'closing';
        else if (lower.includes('player outcome')) current = 'player';
        else if (lower.includes('summary conclusion')) current = 'focus';
        else current = null;
        continue;
      }
...
function generateWinConSummary({
  sideStats,
  closingStats,
  dependenceRows,
}: {
  sideStats: SideIdentityMapStats[];
  closingStats: ClosingStats;
  dependenceRows: PlayerDependenceRow[];
}): string {
...
export async function generateWinconReport(teamId: string, filters: ReportFilters): Promise<Report> {
