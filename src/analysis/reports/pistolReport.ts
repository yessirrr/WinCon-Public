import type { Report, ReportFilters, ReportSection } from '../../data/models/index.js';
import { getTeamById, getPlayerById, getTeamMatches, getPlayerMatches } from '../../data/index.js';

export async function generatePistolReport(
  ...
  const mapPistolStats: Record<string, { played: number; won: number }> = {};
  ...
  for (const match of windowedMatches) {
    for (const map of match.maps) {
      ...
      for (const round of pistolRounds) {
        ...
        if (won) pistolRoundsWon++;

        if (side === 'attacker') {
          attackPistolPlayed++;
          if (won) attackPistolWins++;
        } else {
          defensePistolPlayed++;
          if (won) defensePistolWins++;
        }
        ...
        if (won) mapPistolStats[map.mapName].won++;
      }
    }
  }
