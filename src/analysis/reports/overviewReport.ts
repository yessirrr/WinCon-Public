...
import type { EconIntelData } from '../../ui/components/EconIntelSection.js';
...
export async function generateOverviewReport(
  ...
  const teamId = entityType === 'team' ? entityId : (entity as any).teamId;
  ...
  const momentumData = buildRoundOutcomeReport(
    entityType,
    entityId,
    entityName,
    windowedMatches
  ...
  const econIntelData: EconIntelData = buildEconIntelData(windowedMatches, teamId, filters);
 ...
 */
function buildEconIntelData(
  matches: Awaited<ReturnType<typeof getTeamMatches>>,
  teamId: string,
  filters: ReportFilters
): EconIntelData {
  const mapSummaries: EconIntelData['mapSummaries'] = [];
  ...
  for (const match of matches) {
    for (const map of match.maps) {
      ...
      for (const round of roundObs) {
        ...
        if (isAttack) {
          attackRounds++;
          if (round.winnerTeamId === teamId) attackWins++;
        } else {
          defenseRounds++;
          if (round.winnerTeamId === teamId) defenseWins++;
        }
      }
      ...
      for (const round of roundObs) {
        if (isPistolRound(round.roundIndex)) {
          pistolPlayed++;
          if (round.winnerTeamId === teamId) pistolWins++;
        } else if (isPostPistolRound(round.roundIndex)) {
          postPistolPlayed++;
          if (round.winnerTeamId === teamId) postPistolWins++;
        }
      }
      ...
      for (const [state, pct] of Object.entries(econSummary.econDistribution)) {
        aggEconDist[state as keyof typeof aggEconDist] += pct * mapRounds;
      }
      for (const [buyClass, pct] of Object.entries(econSummary.buyDistribution)) {
        aggBuyDist[buyClass as keyof typeof aggBuyDist] += pct * mapRounds;
      }
    }
  }
  ...
  if (totalRounds > 0) {
    for (const state of Object.keys(aggEconDist) as (keyof typeof aggEconDist)[]) {
      aggEconDist[state] /= totalRounds;
    }
    for (const buyClass of Object.keys(aggBuyDist) as (keyof typeof aggBuyDist)[]) {
