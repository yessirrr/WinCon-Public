...
import { PAUSE_MOMENTUM_WEIGHT, PAUSE_LEVERAGE_WEIGHT, PAUSE_ECON_RISK_WEIGHT, PAUSE_SIDE_SWAP_WEIGHT, PAUSE_RECOMMENDATION_THRESHOLD, FIRST_HALF_END, } from './econModelConfig.js';
function trackScores(rounds, teamId) {
...
// ============================================================================
function calculateOpponentMomentum(rounds, teamId, beforeRound) {
...
// ============================================================================
function calculateLeverage(scores, roundIndex) {
...
// ============================================================================
function calculateEconRisk(econSummary, roundIndex) {
    if (!econSummary)
...
// Side Swap Value
// ============================================================================
function calculateSideSwapValue(roundIndex) {
 ...
 */
export function calculatePauseScore(rounds, teamId, beforeRound, econSummary) {
    ...
    const pauseScore = Math.round((momentum * PAUSE_MOMENTUM_WEIGHT +
    ...
    const reasons = [];
    if (momentum >= 50)
        reasons.push(`Opponent momentum (${Math.round(momentum)})`);
    if (leverage >= 50)
        reasons.push(`High leverage (${Math.round(leverage)})`);
    if (econRisk >= 60)
        reasons.push(`Economy risk (${Math.round(econRisk)})`);
    if (sideSwap >= 40)
        reasons.push('Near side swap');
    return {
 ...
 */
export function analyzeMapPauses(rounds, teamId, econSummary) {
    const scores = trackScores(rounds, teamId);
    const windows = [];
