...
function getRoundType(roundNumber) {
    if (PISTOL_ROUNDS.includes(roundNumber))
        return 'pistol';
    if (POST_PISTOL_ROUNDS.includes(roundNumber))
        return 'post-pistol';
    return 'gun';
}
function getSide(sideStr) {
...
// Timeline Builder
// =============================================================================
function buildTimeline(match, map, teamId) {
    ...
    if (teamStats && opponentStats) {
        if (teamStats.score > opponentStats.score)
            mapWinner = teamId;
        else if (opponentStats.score > teamStats.score)
            mapWinner = opponentId;
    }
    return {
        matchId: match.id,
...
// Side Performance
// =============================================================================
function computeSidePerformance(timelines, teamId) {
    const byMap = new Map();
...
// Pistol Performance
// =============================================================================
function computePistolPerformance(timelines, teamId) {
    const byMap = new Map();
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
        ...
        for (const round of timeline.rounds) {
            ...
            if (teamSide === 'Attack') {
                ...
                if (teamWon)
                    stats.attackPistolWins++;
            }
            else {
                stats.defensePistolPlayed++;
                if (teamWon)
                    stats.defensePistolWins++;
            }
        }
        ...
        if (wonAnyPistol) {
            if (teamWonMap)
                ...
                stats.mapLossesAfterPistolWin++;
        }
        if (lostAnyPistol) {
            if (teamWonMap)
                stats.mapWinsAfterPistolLoss++;
            else
                stats.mapLossesAfterPistolLoss++;
        }
    }
    const results = [];
    for (const [map, stats] of byMap) {
        results.push({
            ...
            mapWinRateGivenPistolWin: safeRate(stats.mapWinsAfterPistolWin, stats.mapWinsAfterPistolWin + stats.mapLossesAfterPistolWin),
            mapWinsGivenPistolLoss: stats.mapWinsAfterPistolLoss,
            mapLossesGivenPistolLoss: stats.mapLossesAfterPistolLoss,
            mapWinRateGivenPistolLoss: safeRate(stats.mapWinsAfterPistolLoss, stats.mapWinsAfterPistolLoss + stats.mapLossesAfterPistolLoss),
        });
    }
    return results.sort((a, b) => a.map.localeCompare(b.map));
}
...
// =============================================================================
function computePostPistolPerformance(timelines, teamId) {
    const byMap = new Map();
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
        ...
        for (const postPistolRound of POST_PISTOL_ROUNDS) {
            ...
            if (teamWonPostPistol)
                stats.postPistolWins++;
            if (teamWonPistol && teamWonPostPistol)
                stats.pistolWinThenPostWin++;
            if (teamWonPistol && !teamWonPostPistol)
                stats.pistolWinThenPostLoss++;
            if (!teamWonPistol && teamWonPostPistol)
                stats.pistolLossThenPostWin++;
            if (!teamWonPistol && !teamWonPostPistol)
                stats.pistolLossThenPostLoss++;
        }
    }
    const results = [];
    for (const [map, stats] of byMap) {
...
// Streak Analysis
// =============================================================================
function computeStreakStats(timelines, teamId) {
    const byMap = new Map(); // map -> array of streak lengths
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
            byMap.set(mapName, []);
...
// =============================================================================
function computeStreakContinuation(timelines, teamId) {
    const byMap = new Map();
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
...
// Half Performance
// =============================================================================
function computeHalfPerformance(timelines, teamId) {
    const byMap = new Map();
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
...
// Heat Zones
// =============================================================================
function computeHeatZones(timelines, teamId) {
    ...
    for (const stats of zoneStats.values()) {
        ...
        if (Math.abs(winRate - avgWinRate) > 0.15)
            significance = 'high';
        else if (Math.abs(winRate - avgWinRate) < 0.05)
            significance = 'low';
        results.push({
...
// Conditional Signals
// =============================================================================
function computeConditionalSignals(timelines, teamId) {
    const byMap = new Map();
    for (const timeline of timelines) {
        const mapName = timeline.mapName;
        if (!byMap.has(mapName)) {
        ...
        const rounds = timeline.rounds;
        // Pistol win
        const wonAnyPistol = rounds.some(r => r.roundType === 'pistol' && r.winnerTeamId === teamId);
        if (wonAnyPistol) {
            stats.pistolWins++;
            if (teamWonMap)
                stats.mapWinsAfterPistolWin++;
        ...
        // Post-pistol win
        const wonAnyPostPistol = rounds.some(r => r.roundType === 'post-pistol' && r.winnerTeamId === teamId);
        if (wonAnyPostPistol) {
            stats.postPistolWins++;
            if (teamWonMap)
                stats.mapWinsAfterPostPistolWin++;
        }
...
// Main Builder
// =============================================================================
export function buildRoundOutcomeReport(entityType, entityId, entityName, matches) {
