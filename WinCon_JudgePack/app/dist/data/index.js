import { isApiConfigured, queryGrid, queryGridState, QUERIES, VCT_TEAMS, getVctTeamsByRegion, findVctTeam, vctRegionToRegion } from './api/gridClient.js';
import { readCache, writeCache, clearAllCacheFiles, listCachedByPrefix } from './cache/fsCache.js';
import { ROSTER_WINDOW_DAYS, CACHE_TTL, isBeforeCutoff } from '../config.js';
...
export { VCT_TEAMS, getVctTeamsByRegion, findVctTeam };
 ...
 */
export async function getRosterMetadata(teamId) {
...
function resolveRegionFromVctTeam(vctTeam) {
    if (!vctTeam)
        return 'NA';
...
function isSchemaFieldError(message) {
    return /FieldUndefined|Cannot query field/i.test(message);
}
async function fetchSeriesState(seriesId) {
    try {
        return await queryGridState(QUERIES.getSeriesState, { id: seriesId });
    }
    catch (error) {
        ...
        if (isSchemaFieldError(message)) {
            ...
            try {
                return await queryGridState(QUERIES.getSeriesStateBase, { id: seriesId });
            }
            catch (fallbackError) {
                ...
                if (isSchemaFieldError(fallbackMessage)) {
                    ...
                    return await queryGridState(QUERIES.getSeriesStateBaseNoObjectives, { id: seriesId });
                }
                throw fallbackError;
            }
        }
 ...
 */
async function fetchSeriesStateRoster(seriesId, targetTeamId) {
    try {
                ...
                if (gameTeam?.players) {
                    for (const gp of gameTeam.players) {
                        ...
                        if (existing && gp.character?.name && !existing.agent) {
                            existing.agent = gp.character.name;
                        }
                    }
                }
            }
 ...
 */
async function buildRosterFromSeriesStates(teamId, teamName, seriesIds, region) {
    ...
    for (const seriesInfo of LIMITED_SERIES) {
        ...
        if (seriesDate && new Date(seriesDate) < windowStart)
            continue;
    ...
    const players = Array.from(participation.values())
 ...
 */
function inferRoleFromAgent(agentName) {
 ...
 */
async function getTeamRosterFromParticipation(teamId, teamName, region) {
    const cacheKey = `roster-inferred-${teamId}`;
    const cached = readCache(cacheKey, true); // Allow expired in demo mode
    if (cached)
        return cached;
 ...
 */
async function enrichRolesFromPlayerSearch(players) {
    ...
    for (const p of players) {
        if (p.role)
            continue;
        try {
            const result = await queryGrid(QUERIES.searchPlayers, {
                name: p.name,
                first: 5,
            });
 ...
 */
export async function fetchTeamFromGrid(teamName) {
    ...
    const teamResult = await queryGrid(QUERIES.searchTeams, {
        name: teamName,
        first: 10,
    ...
    const matchingCandidates = teamEdges.filter(edge => {
        const candidateTeam = findVctTeam(edge.node.name) ??
    ...
    const preferred = candidateEdges.find(e => teamNamesMatch(e.node.name, requestedVctTeam.name) ||
    ...
    const scoredMatches = candidateEdges
        ...
        let score = 0;
        if (nameNormalized === normalizedSearchName)
            score += 100;
        if (shortNormalized === normalizedSearchName)
            score += 95;
        if (nameNormalized.includes(normalizedSearchName))
            score += 35;
        if (shortNormalized.includes(normalizedSearchName))
            score += 30;
        if (teamNamesMatch(edge.node.name, requestedVctTeam.name))
            score += 120;
        if (edge.node.nameShortened && teamNamesMatch(edge.node.nameShortened, requestedVctTeam.shortName))
            score += 110;
    ...
    if (scoredMatches[0]?.score > 0) {
        ...
        if (topMatches.length > 1) {
            ...
            candidates.sort((a, b) => b.seriesCount - a.seriesCount ||
    ...
    const resolvedVctTeam = findVctTeam(teamNode.name) ??
    ...
    const inferredRoster = await getTeamRosterFromParticipation(teamNode.id, normalizedDisplayName, region);
    const team = {
        id: teamNode.id,
...
export async function getTeamById(teamId) {
    ...
    const teamResult = await queryGrid(QUERIES.getTeam, { id: teamId });
    if (!teamResult?.team)
        return undefined;
    ...
    const inferredRoster = await getTeamRosterFromParticipation(teamId, normalizedTeamName, region);
    const team = {
        id: teamNode.id,
...
export async function searchPlayerByName(name) {
    ...
    const cached = readCache(cacheKey, true); // Allow expired
    if (cached)
        return cached;
    // Check individual player cache files (pre-populated for offline use)
    // Exclude player-search-* files to avoid duplicates
    const allCachedPlayers = listCachedByPrefix('player-', /^player-search-/);
    ...
    const result = await queryGrid(QUERIES.searchPlayers, { name, first: 20 });
    const edges = result?.players?.edges ?? [];
    if (edges.length === 0)
        return undefined;
    // Find best match
 ...
 */
async function getPlayerTeamAsOfCutoff(playerId, playerName) {
    try {
        // Get the player's current team from GRID
        const playerResult = await queryGrid(QUERIES.searchPlayers, {
            name: playerName,
            first: 5
        });
        const playerNode = playerResult?.players?.edges?.find(e => e.node.id === playerId)?.node;
        if (!playerNode?.team)
            return undefined;
        ...
        const seriesData = await queryGrid(QUERIES.getTeamMatches, { teamId: currentTeamId, first: 5 });
        ...
        for (const edge of seriesEdges) {
            ...
            if (!stateData?.seriesState)
                continue;
            for (const team of stateData.seriesState.teams) {
                const playerOnTeam = team.players?.some(p => p.id === playerId || p.name.toLowerCase() === playerName.toLowerCase());
                if (playerOnTeam) {
        ...
        for (const region of regionsToSearch) {
            ...
            for (const searchTeam of teamsInRegion) {
                if (searchTeam.name === currentTeamName)
                    continue; // Already checked
                // Get team's series
                const teamSearchResult = await queryGrid(QUERIES.searchTeams, {
                ...
                if (!teamNode)
                    continue;
                const teamSeriesData = await queryGrid(QUERIES.getTeamMatches, { teamId: teamNode.id, first: 3 });
...
export async function searchPlayers(query) {
    ...
    if (allCachedPlayers.length > 0) {
        ...
        const matches = allCachedPlayers.filter(p => p.name.toLowerCase().includes(lowerQuery) ||
    ...
    const result = await queryGrid(QUERIES.searchPlayers, { name: query, first: 20 });
    const edges = result?.players?.edges ?? [];
    ...
    const players = resolvedPlayers.filter((p) => p !== undefined);
...
export async function getPlayerById(playerId) {
    ...
    return readCache(cacheKey, true) ?? undefined; // Allow expired in demo mode
}
...
export async function getTeamMatches(teamId, limit = 8) {
    ...
    const altCached = readCache(altCacheKey, true);
    if (altCached)
        return altCached;
    // No cached results - try API if configured
    if (!isApiConfigured()) {
...
export async function getPlayerMatches(playerId) {
    ...
    const filtered = teamMatches.filter(m => m.maps.some(map => map.rounds.some(r => r.playerStats.some(ps => ps.playerId === playerId))));
...
// ---------- Mappers ----------
function mapGridPlayerToPlayer(node) {
    if (!node)
        return undefined;
    if (!node.team)
        return undefined;
    ...
    const region = vctTeam ? vctRegionToRegion(vctTeam.region) : 'NA';
    return {
        id: node.id,
        name: node.nickname,
        teamId: node.team.id,
...
async function fetchRecentSeriesIdsForTeam(teamId, limit) {
    try {
        const data = await queryGrid(QUERIES.getTeamMatches, {
            teamId,
            first: limit,
        });
...
async function getSeriesMatch(seriesInfo) {
    const id = typeof seriesInfo === 'string' ? seriesInfo : seriesInfo.id;
    const cacheKey = `${SERIES_CACHE_PREFIX}${id}`;
    const cached = readCache(cacheKey);
    if (cached) {
        const needsPlant = cached.maps.some(map => map.rounds.some(round => round.plantOccurred === undefined));
        if (!needsPlant) {
            return cached;
        }
    }
    ...
    try {
        ...
        if (data?.seriesState) {
            const match = mapSeriesStateToMatch(id, data.seriesState, typeof seriesInfo === 'string' ? undefined : seriesInfo);
            // Log what we got
            const roundCount = match.maps.reduce((sum, m) => sum + m.rounds.length, 0);
            const playerStatsCount = match.maps.reduce((sum, m) => sum + m.rounds.reduce((rs, r) => rs + r.playerStats.length, 0), 0);
            if (GRID_VERBOSE) {
                if (roundCount > 0) {
    ...
    try {
        let seriesData = null;
        try {
            seriesData = await queryGrid(QUERIES.getSeriesFull, { id });
        }
        catch (err) {
            ...
            if (isSchemaFieldError(message)) {
                ...
                seriesData = await queryGrid(QUERIES.getSeries, { id });
            }
            else {
                throw err;
            }
...
function mapSeriesStateToMatch(seriesId, state, meta) {
    ...
    const winner = teams.find(t => state.teams.find(st => st.id === t.teamId)?.won)?.teamId ??
        (teams.reduce((prev, curr) => (curr.score > prev.score ? curr : prev), teams[0] ?? { teamId: '', teamName: '', score: 0 }).teamId ||
            undefined);
    const maps = (state.games ?? []).map((g, idx) => {
        ...
        for (const segment of g.segments ?? []) {
            ...
            const hasPlantObjective = (objectives) => (objectives ?? []).some(obj => {
                const lower = (obj.type || '').toLowerCase();
                const sum = obj.completionCount?.sum ?? 0;
                return sum > 0 && (lower === 'plantbomb' ||
            ...
            if (winnerId && side[winnerId]) {
                ...
                if (stats) {
                    if (side[winnerId] === 'attacker')
                        stats.attackRoundsWon += 1;
                    if (side[winnerId] === 'defender')
                        stats.defenseRoundsWon += 1;
                }
            }
        }
        return {
    ...
    if (process.env.WINCON_DEBUG === '1') {
        console.log(`[WINCON] Series ${seriesId} objectives: segments=${debugObjectives.segmentsTotal}, withObjectives=${debugObjectives.segmentsWithObjectives}, plantRounds=${debugObjectives.segmentsWithPlant}`);
    }
    return {
        id: seriesId,
        seriesId,
...
function mapGridSeriesToMatch(series) {
    const teams = (series.teams || []).map(t => ({
        teamId: t.baseInfo.id,
        teamName: normalizeDisplayTeamName(t.baseInfo.name),
        score: t.score ?? 0,
    }));
    const maps = (series.games || []).map((g, idx) => mapGameToMapResult(g, idx + 1));
    const winner = teams.find(t => t.score === Math.max(...teams.map(tm => tm.score ?? 0)))?.teamId ||
...
function mapGameToMapResult(game, fallbackSeq) {
    const teamStats = (game.teams || []).map(t => ({
        teamId: t.baseInfo.id,
        score: t.score ?? 0,
        attackRoundsWon: 0,
        defenseRoundsWon: 0,
    ...
    for (const segment of game.segments ?? []) {
        for (const round of segment.rounds ?? []) {
            ...
            const playerStats = (round.playerStats ?? []).map(ps => ({
            ...
            const existingPlantOccurred = round.plantOccurred;
            rounds.push({
    ...
    const mapWinner = game.teams?.find(t => t.won)?.baseInfo.id ??
...
function determineMapWinner(teamStats) {
...
function normalizeWinCondition(condition) {
    const value = (condition || '').toLowerCase();
    if (value.includes('defuse'))
        return 'spike_defuse';
    if (value.includes('deton'))
        return 'spike_detonation';
    if (value.includes('time'))
        return 'time_expired';
    return 'elimination';
}
function derivePlantOccurredFromWinningCondition(condition) {
    const wc = (condition || '').toLowerCase();
    if (wc.includes('defuse') && wc.includes('bomb'))
        return true;
    if (wc.includes('explode') && wc.includes('bomb'))
        return true;
    if (wc === 'defusebomb' || wc === 'explodebomb')
        return true;
    if (wc.includes('spike') && (wc.includes('defuse') || wc.includes('deton') || wc.includes('explode')))
        return true;
    return false;
}
function classifyEquipment(loadoutValue, roundNumber) {
    if (roundNumber === 1 || roundNumber === 13)
        return 'pistol';
    if (loadoutValue < 2500)
        return 'eco';
    if (loadoutValue < 4000)
        return 'semi-buy';
    return 'full-buy';
}
